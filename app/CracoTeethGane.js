...
const CHOMP_SOUND_URI = require("../assets/music/bite.mp3");
const WIN_SOUND_URI = require("../assets/music/win1.mp3");
...
export default function CracoTeethGame() {
  ...
  const chompPlayer = useAudioPlayer(CHOMP_SOUND_URI);
  ...
  const selectFocused = () => {
    if (focusedId === "minus")
      return noOfTeeth > 2 && setNoOfTeeth(noOfTeeth - 1);
    if (focusedId === "plus")
      return noOfTeeth < 32 && setNoOfTeeth(noOfTeeth + 1);
  ...
  const moveFocus = (direction) => {
    ...
    if (focusedId === "start" || focusedId === "back") {
      if (focusedId === "start" && direction === "right")
        return setFocusedId("back");
      if (focusedId === "back" && direction === "left")
        return setFocusedId("start");
    ...
    if (typeof focusedId === "number") {
      ...
      if (direction === "down" && newIdx >= numArr.length)
        return setFocusedId("start");
...
function ChompOverlay({ triggerKey, active, fontScale }) {
  ...
  return (
    <>
      <Animated.View
        pointerEvents="none"
        style={[styles.chompFlash, flashStyle]}
      />
      <Animated.View style={[styles.chompBar, barStyle]}>
        <Text style={[styles.chompText, { fontSize: 22 * fontScale }]}>
          CHOMP!
        </Text>
      </Animated.View>
    </>
  ...
  return (
    ...
    >
      <Animated.View style={[styles.stepperBtn, animatedStyle]}>
        <Text style={[styles.stepperText, { fontSize: 20 * fontScale }]}>
          {symbol}
        </Text>
      </Animated.View>
    </Pressable>
...
function ActionButton({
  ...
  return (
    ...
    >
      ...
      >
        <Text style={[styles.actionBtnText, { fontSize: 16 * fontScale }]}>
          {label}
        </Text>
      </Animated.View>
    </Pressable>
