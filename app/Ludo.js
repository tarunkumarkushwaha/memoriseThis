...
const WIN_SOUND = require("../assets/music/win1.mp3");
const CAPTURE_SOUND_URI = require("../assets/music/lose1.mp3");
const MOVE_SOUND = require("../assets/music/move.mp3");

const LUDO_SAVE_KEY = "ludo_saved_game";
...
export default function LudoGame() {
  ...
  const moveSound = useAudioPlayer(MOVE_SOUND);
  const captureSound = useAudioPlayer(CAPTURE_SOUND_URI);
  ...
  useEffect(() => {
    AsyncStorage.getItem(LUDO_SAVE_KEY)
      .then((raw) => {
        if (!raw) return;
        try {
  ...
  const saveGameState = useCallback(
    (freshTokens, freshCurrentPlayerIdx) => {
      const payload = {
        ...
        playerTypes,
        tokens: freshTokens,
        currentPlayerIdx: freshCurrentPlayerIdx,
      };
      AsyncStorage.setItem(LUDO_SAVE_KEY, JSON.stringify(payload)).catch(
        () => {},
      );
    },
    [numPlayers, humanColor, playerTypes],
  );

  const clearSavedGame = useCallback(() => {
    AsyncStorage.removeItem(LUDO_SAVE_KEY).catch(() => {});
  }, []);

  const resumeSavedGame = () => {
    if (!pendingSave) return;
    setNumPlayers(pendingSave.numPlayers);
    setHumanColor(pendingSave.humanColor);
    setPlayerTypes(pendingSave.playerTypes);
    setTokens(pendingSave.tokens);
    setCurrentPlayerIdx(pendingSave.currentPlayerIdx);
    setSixStreak(0);
    setWinner(null);
    setMessage("");
  ...
  const declineSavedGame = () => {
    clearSavedGame();
    setShowResumePrompt(false);
    setPendingSave(null);
    setFocusedId("setup-start");
  };
  ...
  const advanceTurn = (bonus, freshTokens = tokens) => {
    if (bonus) {
      setPhase("awaitingRoll");
      setFocusedId("roll");
      saveGameState(freshTokens, currentPlayerIdx);
      return;
    }
    const nextPlayerIdx = (currentPlayerIdx + 1) % numPlayers;
    setSixStreak(0);
  ...
  const selectFocused = () => {
    if (showResumePrompt) {
      if (focusedId === "resume-yes") return resumeSavedGame();
      if (focusedId === "resume-no") return declineSavedGame();
      return;
    }
    if (phase === "setup") {
  ...
  const moveFocus = (direction) => {
    if (showResumePrompt) {
      if (focusedId === "resume-yes" && direction === "right")
        setFocusedId("resume-no");
      else if (focusedId === "resume-no" && direction === "left")
        setFocusedId("resume-yes");
      return;
    }
    if (phase === "setup") {
        ...
        {showResumePrompt ? (
          <Animated.View
            ...
            style={[
              styles.glassCard,
              styles.resumeCard,
              { maxWidth: isTabletOrTV ? 480 : "88%" },
            ]}
          >
            <MaterialCommunityIcons name="restore" size={34} color="#facc15" />
            <Text style={[styles.title, { fontSize: 20 * fontScale }]}>
              Continue Previous Game?
            </Text>
            <Text style={[styles.resumeSubtext, { fontSize: 13 * fontScale }]}>
              You have an unfinished Ludo match ({pendingSave?.numPlayers || 4}{" "}
              players).
            </Text>
            <View style={styles.setupActionGroup}>
              <ActionButton
                ...
                label="Yes, Continue"
                fontScale={fontScale}
                isFocused={focusedId === "resume-yes"}
                onFocusId={setFocusedId}
                onPress={resumeSavedGame}
              />
              <ActionButton
                id="resume-no"
                label="No, New Game"
                variant="secondary"
                fontScale={fontScale}
                isFocused={focusedId === "resume-no"}
                onFocusId={setFocusedId}
                onPress={declineSavedGame}
              />
            </View>
          </Animated.View>
        ) : phase === "setup" ? (
