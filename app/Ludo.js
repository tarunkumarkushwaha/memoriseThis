...
const WIN_SOUND_URI = "https://cdn.pixabay.com/download/audio/2022/03/15/audio_b8c9103636.mp3?filename=correct-83487.mp3";
const CAPTURE_SOUND_URI = "https://cdn.pixabay.com/download/audio/2021/08/04/audio_c6ccf3232f.mp3?filename=negative_beeps-6008.mp3";
...
const SLOT_OFFSETS = [[1.5, 1.5], [1.5, 3.5], [3.5, 1.5], [3.5, 3.5]];
const TOKEN_COLORS = { red: "#ef4444", green: "#22c55e", yellow: "#eab308", blue: "#3b82f6" };
const YARD_TINT = {
...
const PLAYER_PATH = Object.fromEntries(ALL_COLORS.map((c) => [c, buildColorPath(c)]));
...
export default function LudoGame() {
  ...
  // Explicit padding allowance (glassCard padding + boardWrapper's own
  // +8) instead of a raw percentage, so the board can't overflow its
  // card on narrow phones.
  const cardPaddingAllowance = isTabletOrTV ? 64 : 48;
  const maxAvailableSize = Math.min(width - cardPaddingAllowance, height * 0.55);
  ...
  const resolveMove = (color, tokenIdx) => {
    ...
    const { tokens: nextTokens, captured, reachedHome } = applyMove(tokens, color, tokenIdx, roll);
  ...
  useEffect(() => {
    ...
    // diceValue/movableTokens/tokens are intentionally omitted: this
    // effect is meant to re-run only on phase/player transitions, and by
    // the time phase flips to 'awaitingTokenSelect' those values have
    // already updated in the SAME batch, so the closure sees fresh data.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, currentPlayerIdx, isCurrentCPU]);
 
  // Fixed: previously only "setup-start"/"back" did anything here — every
  // count/color/seat button was completely unreachable via D-pad select,
  // only touch worked. Now dispatches based on the focusedId prefix.
  const selectFocused = () => {
    if (phase === "setup") {
      if (focusedId === "setup-start") return startGame();
      if (focusedId === "back") return goBackToMenu();
      if (typeof focusedId === "string" && focusedId.startsWith("count-")) {
        return setNumPlayers(Number(focusedId.split("-")[1]));
      }
      if (typeof focusedId === "string" && focusedId.startsWith("color-")) {
        return setHumanColor(focusedId.split("-")[1]);
      }
      if (typeof focusedId === "string" && focusedId.startsWith("seat-")) {
        const idx = Number(focusedId.split("-")[1]);
        return setPlayerTypes((prev) => {
          const copy = [...prev];
  ...
  const moveFocus = (direction) => {
    if (phase === "setup") {
      const setupGraph = {
        ...
        "color-green": { up: "count-3", left: "color-red", right: "color-yellow", down: "seat-1" },
        "color-yellow": { up: "count-4", left: "color-green", right: "color-blue", down: "seat-2" },
        "color-blue": { up: "count-4", left: "color-yellow", down: "seat-3" },
        "seat-0": { up: "color-red", right: "seat-1", down: "setup-start" },
        "seat-1": { up: "color-green", left: "seat-0", right: "seat-2", down: "setup-start" },
        "seat-2": { up: "color-yellow", left: "seat-1", right: "seat-3", down: "setup-start" },
        "seat-3": { up: "color-blue", left: "seat-2", down: "setup-start" },
        "setup-start": { up: "seat-0", down: "back" },
        back: { up: "setup-start" },
      };
      let next = setupGraph[focusedId]?.[direction];
      // Clamp to the actual number of active seats — a seat card past
      // numPlayers isn't rendered, so focus could otherwise land on
      // nothing.
      if (typeof next === "string" && next.startsWith("seat-")) {
        const seatIdx = Number(next.split("-")[1]);
        if (seatIdx >= numPlayers) next = `seat-${numPlayers - 1}`;
      }
      if (next) setFocusedId(next);
      return;
    }
 
    if (phase === "awaitingTokenSelect" && (direction === "left" || direction === "right")) {
      const i = movableTokens.indexOf(Number(String(focusedId).split("-")[1]));
      if (i !== -1) {
        const nextI = direction === "right" ? (i + 1) % movableTokens.length : (i - 1 + movableTokens.length) % movableTokens.length;
  ...
  return (
    <ImageBackground source={backgroundImageAsset} style={styles.mainContainer} imageStyle={styles.bgImageInner} resizeMode="cover">
      ...
      >
        {phase === "setup" ? (
          <View style={[styles.glassCard, { maxWidth: isTabletOrTV ? 600 : "92%", padding: isTabletOrTV ? 28 : 18 }]}>
            <View style={styles.badge}>
              <Text style={[styles.badgeText, { fontSize: 11 * fontScale }]}>Play OTG SETUP</Text>
            </View>
 
            <Text style={[styles.title, { fontSize: 24 * fontScale }]}>Ludo Customisation</Text>
 
            <Text style={[styles.setupSectionLabel, { fontSize: 13 * fontScale }]}>1. PLAYER COUNT</Text>
            ...
            <Text style={[styles.setupSectionLabel, { fontSize: 13 * fontScale }]}>2. YOUR COLOR (P1)</Text>
            <View style={styles.optionRow}>
            ...
            <Text style={[styles.setupSectionLabel, { fontSize: 13 * fontScale }]}>3. SEAT TYPES</Text>
            <View style={styles.seatsGrid}>
              {activeColors.map((color, idx) => (
                <View key={color} style={[styles.seatCard, { borderColor: TOKEN_COLORS[color] }]}>
                  <Text style={[styles.seatColorLabel, { color: TOKEN_COLORS[color], fontSize: 12 * fontScale }]}>
                    {color.toUpperCase()}
                  </Text>
                  <PressableBtn
                    id={`seat-${idx}`}
            ...
            <View style={styles.setupActionGroup}>
              <ActionButton id="setup-start" label="Start Match" fontScale={fontScale} isFocused={focusedId === "setup-start"} onFocusId={setFocusedId} onPress={startGame} />
              <ActionButton id="back" label="Back to Menu" variant="secondary" fontScale={fontScale} isFocused={focusedId === "back"} onFocusId={setFocusedId} onPress={goBackToMenu} />
            </View>
          </View>
        ) : (
          <View style={[styles.glassCard, { maxWidth: isTabletOrTV ? 720 : "95%", padding: isTabletOrTV ? 20 : 12 }]}>
            <TurnBanner color={currentColor} isCPU={isCurrentCPU} phase={phase} message={message} fontScale={fontScale} />
            <View style={[styles.boardWrapper, { width: BOARD + 8, height: BOARD + 8 }]}>
                ...
                {activeColors.map((color) =>
                  tokens[color]?.map((pos, idx) => (
                    <Token
                      ...
                      isMovable={color === currentColor && !isCurrentCPU && movableTokens.includes(idx) && phase === "awaitingTokenSelect"}
                      isFocused={focusedId === `token-${idx}` && color === currentColor}
                      onPress={() => {
                        if (color === currentColor && !isCurrentCPU && movableTokens.includes(idx) && phase === "awaitingTokenSelect") {
            ...
            <View style={styles.controlsContainer}>
              <Pressable
                ...
                onPress={rollDice}
              >
                <DiceFace value={diceValue} isFocused={focusedId === "roll"} isRolling={phase === "rolling"} disabled={phase !== "awaitingRoll" || isCurrentCPU} fontScale={fontScale} />
              </Pressable>
 
              {phase === "gameOver" && winner && (
                <Animated.View entering={FadeInDown.duration(300)} style={styles.winnerBox}>
                  <Text style={[styles.winnerText, { color: TOKEN_COLORS[winner], fontSize: 18 * fontScale }]}>{winner.toUpperCase()} WINS!</Text>
                  <ActionButton id="play-again" label="Play Again" fontScale={fontScale} isFocused={focusedId === "play-again"} onFocusId={setFocusedId} onPress={startGame} />
                </Animated.View>
              )}
 
              <ActionButton id="back" label="Menu" variant="secondary" fontScale={fontScale} isFocused={focusedId === "back"} onFocusId={setFocusedId} onPress={goBackToMenu} />
            </View>
          </View>
        )}
      </ScrollView>
...
function BoardCells({ cell, activeColors }) {
    ...
    items.push(
      <View
        ...
        style={{
          ...
          width: 6 * cell,
          height: 6 * cell,
          backgroundColor: isActive ? YARD_TINT[color] : "rgba(255,255,255,0.02)",
          borderWidth: 2,
          borderColor: isActive ? TOKEN_COLORS[color] : "rgba(255,255,255,0.08)",
          borderRadius: 14,
          padding: cell * 0.4,
        }}
      >
        <View style={{ flex: 1, backgroundColor: "rgba(15, 23, 42, 0.65)", borderRadius: 10, borderWidth: 1, borderColor: "rgba(255,255,255,0.1)" }} />
      </View>
    );
  });
  ...
  GLOBAL_PATH.forEach(([r, c], i) => {
    ...
    const starColor = i === 1 ? "#ef4444" : i === 14 ? "#22c55e" : i === 27 ? "#eab308" : i === 40 ? "#3b82f6" : "#f59e0b";
    items.push(
      <View
        key={`path-${i}`}
        style={{
          ...
          width: cell,
          height: cell,
          backgroundColor: isSafe ? "rgba(253, 224, 71, 0.25)" : "rgba(248, 250, 252, 0.92)",
          borderWidth: 0.5,
      ...
      >
        {isSafe && <FontAwesome5 name="star" size={cell * 0.45} color={starColor} solid />}
      </View>
    );
  });
...
function Token({ color, position, tokenIdx, cell, allTokens, activeColors, isMovable, isFocused, onPress }) {
  ...
  const stackOffset = useMemo(() => {
    ...
    return { dx: (myRank % 2) * shift - shift / 2, dy: Math.floor(myRank / 2) * shift - shift / 2 };
  }, [position, color, tokenIdx, allTokens, activeColors, cell]);
 
  useEffect(() => {
    const [row, col] = position === 0 ? yardSlot(color, tokenIdx) : cellForToken(color, position) || yardSlot(color, tokenIdx);
    const targetX = col * cell + (cell - size) / 2 + stackOffset.dx;
    const targetY = row * cell + (cell - size) / 2 + stackOffset.dy;
 
    arcY.value = withSequence(withTiming(-14, { duration: 140 }), withTiming(0, { duration: 160 }));
  ...
  return (
    <Pressable onPress={onPress} style={{ position: "absolute" }} hitSlop={6}>
      <Animated.View style={[styles.token, animatedStyle, { backgroundColor: TOKEN_COLORS[color], borderColor: isFocused ? "#ffffff" : "rgba(255,255,255,0.85)" }]}>
...
function DiceFace({ value, isFocused, isRolling, disabled, fontScale }) {
  ...
  useEffect(() => {
    if (isRolling) {
      ...
      scale.value = withSequence(withSpring(1.25, BOUNCE_SPRING), withSpring(1, BOUNCE_SPRING));
    }
  }, [isRolling, value]);
  ...
  return (
    <Animated.View style={[styles.dice, animatedStyle, disabled && { opacity: 0.5 }]}>
      <FontAwesome5 name={DICE_FACES[value - 1]} size={28 * fontScale} color="#0f172a" solid />
    </Animated.View>
  );
}
...
function TurnBanner({ color, isCPU, phase, message, fontScale }) {
  return (
    <Animated.View entering={FadeIn.duration(200)} style={[styles.turnBanner, { borderColor: TOKEN_COLORS[color] }]}>
      <Text style={[styles.turnText, { color: TOKEN_COLORS[color], fontSize: 14 * fontScale }]}>
        {color.toUpperCase()}'S TURN {isCPU ? "(CPU)" : ""}
      </Text>
      {!!message && <Text style={[styles.turnMessage, { fontSize: 11 * fontScale }]}>{message}</Text>}
      {phase === "awaitingTokenSelect" && !isCPU && <Text style={[styles.turnMessage, { fontSize: 11 * fontScale }]}>Select a highlighted token</Text>}
    </Animated.View>
  );
}
 
function PressableBtn({ id, label, active, icon, accentColor, isFocused, fontScale, onFocusId, onPress }) {
  const focusAnim = useSharedValue(0);
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    borderColor: focusAnim.value > 0.05 ? "#ffffff" : active ? accentColor || "#6366f1" : "rgba(255,255,255,0.15)",
  ...
  return (
    <Pressable focusable isTVSelectable onFocus={() => onFocusId(id)} onBlur={() => onFocusId(null)} onPress={onPress}>
      <Animated.View style={[styles.pressableBtn, active && { backgroundColor: accentColor || "rgba(99, 102, 241, 0.3)" }, animatedStyle]}>
        {icon}
        <Text style={[styles.pressableBtnText, { fontSize: 12 * fontScale }]}>{label}</Text>
      </Animated.View>
    </Pressable>
  );
}
...
function ActionButton({ id, label, variant = "primary", fontScale, isFocused, onFocusId, onPress }) {
  ...
  return (
    <Pressable focusable isTVSelectable onFocus={() => onFocusId(id)} onBlur={() => onFocusId(null)} onPress={onPress} style={{ width: "auto", padding: 10 }}>
      <Animated.View style={[styles.actionBtn, variant === "primary" ? styles.actionBtnPrimary : styles.actionBtnSecondary, animatedStyle]}>
        <Text style={[styles.actionBtnText, { fontSize: 14 * fontScale }]}>{label}</Text>
      </Animated.View>
    </Pressable>
  );
}
